...
$(document).ready( function () {
	...
	<?php if(isset($datatableRowGroupBy)){ ?>
	var groupColumn = <?=$datatableRowGroupBy?>;
	var table = $("#example").DataTable({
		<?php if(isset($noSorting)){echo '"ordering": false,';}?>
		<?php if(isset($noSearch)){echo '"searching": false,';}?>
		<?php if(isset($noPaginate)){echo '"bPaginate": false,';}?>
		<?php if(isset($noLengthChange)){echo '"bLengthChange": false,';}?>
		<?php if(isset($noInfo)){echo '"bInfo": false,';}?>
		"columnDefs": [
		...
		"drawCallback": function ( settings ) {
			...
			api.column(groupColumn, {page:"current"} ).data().each( function ( group, i ) {
				if ( last !== group ) {
					$(rows).eq( i ).before(
						'<tr class="group"><td bgcolor="#eee" colspan="<?=$datatableNumCol-1?>">'+group+'</td></tr>'
					);
					last = group;
				}
			});
	...
	<?php }else{ ?>
  $('.data-table').DataTable(<?php if(isset($dataTableParam)){ echo $dataTableParam; } ?>);
	<?php } ?>
		
	<?php if(isset($showModal)){ ?> $('#myModal').modal('show');  <?php } ?>
} );
		</script>
